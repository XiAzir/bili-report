...
**推荐使用 Web UI 操作**：`npm run ui` 启动本地工作台，在浏览器中完成采集→规范化→切片→合并→举报全流程，无需记忆命令行参数。

支持两种使用方式：**Web UI**（推荐，全流程可视化操作）或 **CLI**（命令行）。
...
5. 点击「举报」，填写动态 OID（即动态 ID），点击「开始举报」提交
...
├── merger/             合并已审核条目
├── reporter/           举报提交
├── ui/                 Web UI 工作台（采集/规范化/切片/合并/举报 全流程可视化入口）
└── shared/
